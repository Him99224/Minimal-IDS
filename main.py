...
from detectors.session_layer import (
    check_brute_force,
...
async def intrusion_detection_middleware(request: Request, call_next):
    ...
    if token:
        try:
            ...
            if threat_type:
                record_threat(user_id, client_ip, "SESSION_HIJACKING")
                add_to_blacklist(jti, float(payload["exp"]), reason="session-hijack")
        except HTTPException:
            # Ignore here so route-level auth can return standard responses.
            pass
...
def login(data: LoginRequest, request: Request):
    ...
    if not user or user["password"] != data.password:
        ...
        if threat_type:
            record_threat("unknown", client_ip, "BRUTE_FORCE")
